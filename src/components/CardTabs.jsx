...
const TabHeader = ({ selected, children, ...props }) => {
  return <TabHeaderContainer bg={selected ? 'white' : 'lightGrey' } py='m' flexGrow='1' textAlign='center' borderBottom={selected ? null : 'default'} {...props}>
...
const CardTabs = ({ headers, children }) => {
  return <Card><Tabs header={<Flex>{ headers.map((header) => <TabHeader>{ header }</TabHeader>) }</Flex>}>
    { children }
  </Tabs></Card>
}
